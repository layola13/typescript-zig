 ...
 */
function buildZts(opts) {
    opts ||= {};
    return $({ cancelSignal: opts.abortSignal })`zig build -p ./built/local ${opts.extraArgs ?? []}`;
}
...
export const ztsBuild = task({
    name: "zts:build",
    description: "Builds the zts Zig prototype binary.",
    run: async () => {
        await buildZts({
            extraArgs: options.release ? ["-Doptimize=ReleaseFast"] : ["-Doptimize=Debug"],
        });
    },
});
        ...
        await watchDebounced("build:watch", async (paths, abortSignal) => {
            ...
            if (paths) {
                for (const p of paths) {
                    ...
                    else if (p.endsWith(".zig") || p.endsWith("build.zig")) {
                        zigChanged = true;
                    }
                    if (libsChanged && goChanged && zigChanged) {
                        break;
                    }
                }
            }
            ...
            paths: ["cmd", "internal", "zig", "build.zig"],
